    ...
    <div class="server-list">
        ...
        <el-table-column prop="serverName" label="项目名" width="120">
          <template #default="scope">
            <div class="table-cell">
              <el-button type="text" plain @click="getServerList(scope.row)">
      ...
      <el-table :data="list.serverList" style="width: 100%" stripe>
        <el-table-column prop="name" label="服务名" min-width="120">
          <template #default="scope">
            <div class="table-cell">
              <el-button type="text" plain @click="showServerDetail(scope.row)">
                {{ scope.row.name }}
              </el-button>
            </div>
          </template>
      ...
      <el-table :data="po.serverPo.classList" style="width: 100%" stripe>
        <el-table-column prop="className" label="类名" min-width="120">
          <template #default="scope">
            <div class="table-cell">
              <el-button type="text" plain @click="showMethodDetail(scope.row)">
                {{ scope.row.name }}
              </el-button>
            </div>
          </template>
        </el-table-column>
        <el-table-column prop="path" label="类路径" min-width="120"></el-table-column>
      ...
      <el-table :data="po.classList.methodList" style="width: 100%" stripe>
        <el-table-column prop="name" label="方法" min-width="120">
          <template #default="scope">
            <div class="table-cell">
              <el-button type="text" plain @click="showMethod(scope.row, po.classList)">
                {{ scope.row.name }}
              </el-button>
            </div>
          </template>
        </el-table-column>
        <el-table-column prop="deal" label="属性" width="80">
          <template #default="scope">
      ...
      <div style="width: 100%; margin-top: 20px;">
        ...
        <div>
          <el-form :model="po.dealMethodPo" style="margin-top: 10px;">
            ...
            <el-form-item>
              <el-button type="primary" @click="dealMethod" size="mini">执行</el-button>
            </el-form-item>
          </el-form>
        </div>
        <div>
          <div>
            <el-descriptions title="" size="mini">
              <el-descriptions-item v-if="po.dealMethodPo.result != null" label="结果状态码">
                ...
                <el-tag v-else class="ml-2" type="danger" size="mini">
                  {{ po.dealMethodPo.result.code }}
                </el-tag>
              </el-descriptions-item>
              <el-descriptions-item v-else label="结果状态码">
                <el-tag class="ml-2" type="success" size="mini">0</el-tag>
              </el-descriptions-item>
          ...
          <div label="结果数据">
            <el-input v-if="po.dealMethodPo.result != null" type="textarea" v-model="po.dealMethodPo.result.dataJson" placeholder="data" clearable :rows="6" size="mini"/>
            <el-input v-else type="textarea" placeholder="data" clearable :rows="6" size="mini"/>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    dealMethod() {
      // this.po.dealMethodPo.data = this.po.dealMethodPo.jsonData
